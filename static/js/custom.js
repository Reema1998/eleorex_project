...
$(document).ready(function() {
    ...
    $('#questionList').sortable({
        ...
        stop: function() {
           ...
           var questionDtl = {};
           $('#questionList .eachQuestionContainer').each(function(){
                var timeout = $(this).find('.time').html();
                var question = $(this).find('.question').html();
                questionDtl[ques_count] = { "time": timeout, "question": question };
                $(this).find('.qnNo').html(ques_count++);
           });
    ...
    $('#addQuestion').click(function() {
        ...
        if (errorMsg != "") {
            ...
            var elemnt = '<div class="eachQuestionContainer" ><div class="row" ><div class="col-md-1 col-sm-1 col-xs-12 text-center" ><span class="qnNo" >' + count + '.</span></div><div class="col-md-8 col-sm-8 col-xs-12 Qn" ><p><span class="question">' + question + "</span> (<span class='time'>" + timeout + '</span> seconds)<span class="text-danger" title="Delete" ><i class="fas fa-trash-alt deleteQnFromList"></i></span></p></div></div></div>';
            $('#questionList').append(elemnt);
...
function addInterviewQuestions() {
    ...
    $('#questionList').append(appendString);
    $('#qbj_question').val(JSON.stringify(questionDtl));
    var checkVal = $('#qbj_question').val();
    if (checkVal.length) {
        $('.noQn').hide();
...
$(document).ready(function() {
    ...
    $(document).on("click", ".deleteQnFromList", function() {
        if (confirm("Are you sure?")) {
            $(this).parents('.eachQuestionContainer').remove();
...
$('#searchUser').on('keyup', function() {
    ...
    // }).show();
        container = $("#shareToUsers");
        li = container.find("option");
